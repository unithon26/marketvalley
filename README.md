...
현재 저장소에는 제품 문서와 개발환경만 있다. 이전에 만든 임시 화면, mock 구현, API, fixture와 화면 테스트는 제품 디자인 기준으로 사용하지 않기 위해 모두 제거했다. 웹페이지 구현은 별도 랜딩 저장소의 시각 요소와 디자인 담당자가 전달할 메인 웹사이트 디자인을 확인한 뒤 시작한다.
...
공개 캠페인 랜딩의 시각 요소 참고용 코드는 별도 저장소 `https://github.com/unithon26/landing-page-reference`에서 관리한다. 구조와 기능은 이 저장소의 스펙을 따르고, 메인 웹사이트 화면은 디자인 담당자의 확정본을 받은 뒤 구현한다.

## 구현 시작 조건

다음 입력이 모이기 전에는 메인 웹사이트 화면을 만들지 않는다.

1. 시각 요소만 참고할 랜딩페이지 저장소의 기준 브랜치
2. 디자인 담당자의 메인 웹사이트 최종 프레임과 상태별 화면
3. 실제 개발자 이름과 파일 소유권
4. 발표 시간, 제출 형식과 심사 기준

화면 구현을 시작할 때는 참고 저장소의 시각 요소와 확정 디자인을 먼저 정리하고 디자인 토큰과 컴포넌트 경계를 확정한 뒤 문서의 G0부터 순서대로 진행한다.
