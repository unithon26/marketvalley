...
- 동일 Meta 광고계정에는 live run 하나만 허용한다.
...
- 박지성: 제품 UI, 결정적 renderer, export, E2E, 배포
- 홍성주: `CampaignSpec`, AI, Supabase·RLS, API, lifecycle
- 디자인: Figma 기반 랜딩·카드뉴스·제품 화면 시스템

## 현재 한계

marketvalley는 작은 표본의 첫 행동 신호를 모으는 도구다. 통계적으로 유의한 시장조사, 광고 최적화나 사업 성공 판정을 대신하지 않는다. Meta OAuth, 광고계정·예산 승인과 결제수단 설정에는 사람이 남는다. 최종 리포트는 실제 수집 구간이 끝난 뒤에만 완료된다.
