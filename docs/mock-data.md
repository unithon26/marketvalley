...
- 랜딩, 인스타그램 카드와 예약자명단을 한 흐름에서 이해하기 쉽다.
...
템플릿 ID는 `CampaignSpec.templates`에 저장되며 브랜드 tone에서 추론하지 않는다. Anthropic adapter도 지원 목록 밖의 ID를 반환할 수 없고 최종 Zod 검증에서 다시 거절한다.
...
mock 저장소는 발표 시작 상태에 `example.com` 이메일을 쓰는 합성 seed 예약 4건을 포함한다. 실제 사용자나 실제 조사 데이터가 아니며 목록 화면에서는 이메일을 마스킹한다.

- 이서준, 박하늘, 최민서, 정다인의 합성 이름
- 2026-08-24 09:00부터 5분 간격의 결정적 접수 시각
- 실제 개인정보가 아닌 fixture 예약 record

발표 중 예약 한 건을 제출하면 사전 최소 표본 5명에 도달한다. 화면은 이를 `데모 데이터`로 표시하며 발표 중 제출한 예약도 실제 사용자 조사로 주장하지 않는다. 초기화하면 seed 4건과 `nextAction: null` 상태로 돌아간다.

## Anthropic live 회귀 입력
