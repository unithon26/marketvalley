...
- **입력 반영 범위 — 결정됨:** fixture 생성기는 자유 텍스트를 생성하지 않는다. 저장된 레퍼런스 `CampaignSpec` 3개(`lib/demo/referenceTemplates.ts`) 중 입력 텍스트의 키워드와 가장 잘 맞는 것을 고르고, 그 템플릿의 데이터셋을 그대로 쓴다. OpenAI 연동 이후에도 "여러 레퍼런스 중 선택"이라는 구조는 유지하고, 선택 로직만 키워드 매칭에서 실제 모델 호출로 바뀐다. Meta 게시는 여전히 "게시 준비 파일"까지만 — 실제 Meta API 게시는 범위 밖이다.
