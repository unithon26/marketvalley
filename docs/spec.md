...
Figma 발표 흐름은 별도 가설 승인 화면을 두지 않는다. `/new`의 두 번째 입력을 제출하면 입력 화면을 즉시 `접수 → 준비 중 → 수집 중 → 결과 도착` 진행 화면으로 바꾼다. 카드뉴스·랜딩페이지 제작은 실제 `/api/generate` 응답까지, 시장 데이터 수집 표시는 실제 `/api/campaigns` 게시와 발표용 수집 표현이 끝날 때까지 해당 단계에 머문다. 그 뒤에만 `/campaigns/[id]` 결과 화면으로 자동 이동한다.
