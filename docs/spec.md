...
제품은 `아이디어 입력 → 캠페인 자동 게시 → 공개·내보내기 → 관심 응답 회수 → 사람의 다음 판단`이라는 한 경로만 깊게 완성한다. 자유도, 채널 수, 템플릿 수보다 메시지 일관성, 실제 URL, 실제 응답 한 건, 시각적 완성도와 실패 복구를 우선한다.
...
`/new`의 2단계 온보딩을 제출하면 별도 승인 화면 없이 바로 캠페인을 생성·게시한다. 시스템은 캐러셀·게시 문구 파일을 생성한 뒤 `/campaigns/[id]/progress`를 거쳐 `/campaigns/[id]` 결과 화면으로 이동한다.

랜딩, 캐러셀과 Meta용 사전 확인 화면은 만들지 않는다. 실제 랜딩은 발급된 공개 URL에서 확인하고, 캐러셀은 내려받은 PNG·ZIP이 최종 결과물이다. 내용을 바꾸려면 `/new`로 돌아가 새 캠페인을 생성한다.

완료 기준:

- 온보딩 제출은 중복 실행되지 않으며 생성 또는 게시 실패를 사실대로 구분한다.
...
- 전체 프로젝트 대시보드: 진행 중인 캠페인 카드 그리드, 상태 필터, `+새로 만들기`
- 제품이 없애는 기존 작업을 짧은 `before/after`로 표시

### `/new`

- 2단계 온보딩: 1단계 제품 배경, 2단계 솔루션 설명(각 최소 20자)
- 예시 불러오기로 데모 입력 한 번에 채우기
- 2단계 제출 시 승인 화면 없이 바로 캠페인 생성 요청 후 `/campaigns/[id]/progress`로 이동
...
- `POST /api/generate`: 입력을 검증하고 `CampaignSpec`을 반환한다. 성공 시 클라이언트가 `crypto.randomUUID()`로 `draftId`를 만들고 승인 단계 없이 바로 `POST /api/campaigns`를 호출해 게시한다.
...
- 완료 게이트 G2: 외부 API와 실제 계정 없이 `/new → publish → /campaigns/demo → /p/demo → 응답 → 결과 → 사람의 판단 → PNG/ZIP`이 끝까지 작동한다.
...
- `pnpm test`
- `pnpm build`
- 개발 검증용 E2E 도구로 fixture 기반 `/new → publish → signal → 판단 → ZIP` 핵심 흐름 1개. 발표 자동 클릭 기능은 만들지 않는다.
