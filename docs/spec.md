...
Figma 발표 흐름은 별도 가설 승인 화면을 두지 않는다. `/new`의 두 번째 입력을 제출하면 입력 화면을 즉시 진행 화면으로 바꾼다. 아직 연결하지 않은 시장 조사 준비 단계만 2초 데모로 통과하고, AI 문구 생성은 실제 `/api/generate` 응답까지, 광고 구성은 실제 `/api/campaigns` 게시 완료까지 해당 단계에 머문다. 게시가 끝난 뒤에만 `/campaigns/[id]` 결과 화면으로 자동 이동한다.
...
- 제출 직후 진행 화면으로 전환하고 `시장 조사 준비 2초 → 실제 AI 문구 생성 → 실제 광고 구성 → 결과 도착`을 표시
- AI 생성·광고 게시가 실제 완료된 뒤 `/campaigns/[id]`로 자동 이동

### `/campaigns/[id]/progress`

- 기존 게시 광고의 직접 접근 호환을 위한 결정적 4단계 화면
