...
현재는 화면과 adapter를 구현하지 않았다. 아래 순서는 기존 랜딩페이지 템플릿과 메인 웹사이트 디자인을 반영하고 mock 발표 흐름이 안정된 뒤 실제 데이터를 채우기 위한 작업 기준이다.
...
- GitHub 저장소를 연결하고 preview 배포부터 확인한다.
- mock 모드에는 외부 환경변수를 등록하지 않는다.
- live preview에만 OpenAI·Supabase 값을 암호화된 환경변수로 등록한다.
