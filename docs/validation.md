...
- ADR-0012로 이름·이메일 예약자명단 모델을 채택했다. 전화번호는 여전히 받지 않는다. 이름·이메일은 명시적 동의 체크박스 뒤에만 수집하고, 리스트 표시에는 마스킹된 이메일만 노출한다.
- API 키가 클라이언트 bundle, Git, 로그와 화면에 없다.
- 모든 seed 응답과 fixture는 `데모 데이터`로 표시된다.
- 근거 없는 후기, 사용자 수, 효능·매출 수치와 인증이 없다. 단, ADR-0012로 도입한 노출수·CTR·체류시간·이탈률·예약률 예시 지표는 예외이며, 화면에 "예시 지표" 라벨을 반드시 표시해 실측치와 구분한다.
