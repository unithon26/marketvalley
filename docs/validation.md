...
- `/p/demo`에서 이름·이메일·동의로 사전예약 한 건을 제출할 수 있다.
- 같은 캠페인의 같은 이메일 두 번째 예약은 중복으로 처리된다.
- 예약·판단·초기화 API 실패는 성공으로 표시되지 않으며 재시도할 수 있다.
- 실제 예약자 수·접수 추이와 사전 최소 표본이 사실대로 표시된다.
...
- 근거 없는 후기, 사용자 수, 효능·매출 수치와 인증이 없다. 리포트는 저장된 예약자 수·명단·접수 추이와 사전 판단 기준만 표시하며, 방문 이벤트와 Meta Insights를 연결하기 전에는 노출 수·CTR·체류시간·스크롤 깊이·예약률·인구 통계를 만들지 않는다.
