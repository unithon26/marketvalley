...
상태: fixture 기반 mock 종단 구현 완료, live adapter 미연동
기준일: 2026-08-25
...
| 모드 | 생성 | 공개·응답·판단 | 외부 키 |
| --- | --- | --- | --- |
| `mock` | `FixtureCampaignGenerator` | `FixtureCampaignRepository` | 불필요 |
...
캐러셀 ZIP에는 1080×1350 PNG 5장을 넣는다. `Meta 게시 준비` ZIP에는 동일한 PNG 5장과 기본 문구, headline, CTA, 대상 고객 가설, 시각 방향, 파일 목록과 절대 destination URL을 적은 `meta-ready.txt`를 함께 넣는다. 두 ZIP 모두 같은 숨은 React/CSS 렌더러를 사용하며 실제 Meta 계정에는 쓰지 않는다.
