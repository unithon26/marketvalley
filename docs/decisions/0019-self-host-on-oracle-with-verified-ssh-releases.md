...
OCI는 tenancy당 NLB 하나를 Always Free로 안내하지만 실제 Console 자격과 기존 사용량이 최종 기준이다. 2026-08-25 확인 결과 기존 NLB는 0개이고 현재 청구액은 SGD 0이다. Terraform apply, VNIC NSG append와 서버 bootstrap은 production 변경이므로 검토 직전 plan과 대상 OCID를 다시 확인한다.
