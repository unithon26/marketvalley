...
export const AI_COPY_SLOT_GROUPS: readonly PromptSlotGroup[] = [
  ...
  },
  {
    ...
    instruction: "사용자 입력에 근거해 고객·문제·해결을 분리한다. 추론한 내용은 assumptions에 넣는다. expectedSignal은 이름·이메일 수집 동의 뒤 기록되는 실제 예약 한 건으로 쓴다. 무응답·중단 조건은 서버가 결정하므로 임의의 기간·비율·응답 수를 만들지 않는다.",
  },
  {
  ...
  },
  {
    ...
    instruction: "서로 다른 각도의 후킹 문구 3개를 만든다. hooks[0]은 고객이 겪는 구체적인 반복 순간을 질문이나 대비로, hooks[1]은 사라지는 일과 작동 방식을, hooks[2]는 반드시 운영자가 예약자명단을 보고 다음 실험·개설·판매 여부를 판단한다는 사람의 역할을 자연스러운 한 문장으로 강조한다. hooks[2]를 독자에게 내리는 명령으로 쓰거나 같은 주어를 반복하지 않는다. 각 문구는 70자 이하이며 확인되지 않은 수치·공포·성과 보장을 쓰지 않는다.",
  },
  {
    id: "social-caption",
  ...
  },
  {
    ...
    instruction: "구매 전 가장 먼저 생길 질문 세 개를 고른다. 입력에 없는 가격·효능·일정·환불·지원 채널·개인정보 처리방침 링크를 만들어내지 말고, 알 수 없는 내용은 운영자 확인 필요로 표현한다. 기존 수업의 당일 예약 취소가 문제로 주어졌더라도 새 사전예약의 취소·환불 정책으로 확대하지 않는다. 이름·이메일은 명시적 동의 후 예약자명단 확인 목적으로만 저장되고 예약이 구매나 성과를 보장하지 않는다는 제품 고정 정책만 포함한다.",
  },
  {
    id: "carousel-hook",
