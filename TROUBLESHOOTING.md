...
아이디어 제출 뒤 두 캠페인이 `GENERATING → RETRY_WAIT`로 전환되며 `일시 오류 · 자동 재시도 예약`을 표시했다. worker와 lease는 정상이었지만 한 캠페인은 같은 오류를 세 번 반복해 `FAILED`가 됐고, 사용자는 생성이 멈춘 것으로 보였다. 기존 실제 수집 캠페인에는 영향이 없었다.

### 재현과 증거

- 두 접수의 오류 코드는 모두 `anthropic_unsafe_output`이었고 lease는 해제된 상태였다.
- 가장 최근 입력 한 건을 같은 production prompt와 model로 재현하자 안전성 판정의 `prohibited_claim` 분기에서 거절됐다.
- 입력·생성 문구를 출력하지 않고 규칙과 JSON path만 분류한 결과 위반은 본문이 아니라 `hashtags[4]`의 입력에 없는 시간 절감 표현 하나였다.
- 같은 Structured Outputs 요청을 재실행해도 해당 종류의 해시태그가 다시 만들어질 수 있어 transient backoff만으로는 회복을 보장하지 못했다.

### 원인과 대안

서버는 입력에 없는 가격·채널 해시태그를 먼저 제거했지만 정량 근거 없는 성과 표현 검사는 전체 공개 문구를 합친 뒤 수행했다. 따라서 제거 가능한 해시태그 하나가 랜딩·카드뉴스 전체 생성을 폐기했다. 안전성 검사를 끄거나 fixture 결과로 대체하면 근거 없는 광고를 게시할 수 있어 기각했다. 같은 deterministic 계열의 콘텐츠 오류를 계속 재시도하는 방식도 비용만 늘려 기각했다.

### 해결과 회귀 방지

근거 없는 성과 표현이 해시태그에만 있을 때 해당 태그를 제거하고, 남은 문구 전체에 기존 수치·매출·효능·보장 검사를 다시 적용한다. 모든 태그가 제거되면 안전한 프로젝트명 또는 `#시장검증`을 사용한다. `anthropic_unsafe_output` 자체는 다시 나타날 경우 즉시 `FAILED`로 전환해 무의미한 자동 재시도를 막는다. 단위 테스트는 시간 절감 해시태그만 제거되는 경우, 본문 성과 주장은 계속 거절되는 경우와 영구·일시 오류 분류를 각각 고정한다.

### 검증과 남은 위험

focused 4파일 31개, 전체 단위 테스트 42파일 220개와 production build 기반 Chromium E2E 7개가 통과했다. 운영 배포 뒤 실패 캠페인을 저장된 입력으로 한 번 재개해 `GENERATING`을 벗어나는지 확인한다. 향후 새로운 제거 가능 패턴이 발견되면 전체 생성 거절보다 슬롯별 안전 교정을 우선하되, 사실 주장 본문은 계속 fail-closed로 유지한다.
