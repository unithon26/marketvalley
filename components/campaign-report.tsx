  ...
  return (
    <main className="report-page page-container">
      ...
      <section className="report-section deliverables-section">
        <div className="section-heading"><div><span className="eyebrow">READY TO USE</span><h2>광고 결과물</h2></div><span className="safe-label">{metaAdsEnabled ? "Meta 계정에 PAUSED 초안 생성 · 실제 노출·광고비 지출 없음" : "외부 계정·광고비 사용 없음"}</span></div>
        <div className="deliverable-grid">
          ...
          <article className="meta-deliverable"><div className="deliverable-icon meta-icon">M</div><div><h3>Meta 게시 준비</h3><p>PNG 5장·문구·CTA·대상 고객 가설·절대 URL을 ZIP 하나로</p><code>{metaAdsEnabled ? "Meta 계정의 Ads Manager에 PAUSED 초안 생성 · 실제 노출·광고비 지출 없음" : "실제 게시 또는 집행 아님"}</code></div><div className="deliverable-actions"><button className="icon-button" type="button" onClick={downloadMetaPackage} disabled={exporting} aria-busy={exporting} aria-label="Meta 게시 준비 다운로드"><DownloadIcon /></button>{metaAdsEnabled && <button className="button button-secondary meta-draft-button" type="button" onClick={createMetaPausedDraft} disabled={exporting || ["completed", "quota", "reconciliation"].includes(metaDraftState.kind)} aria-busy={metaDraftState.kind === "creating"}>Ads Manager PAUSED 초안 만들기</button>}</div>{metaAdsEnabled && metaDraftState.message && <p className={`meta-draft-status meta-draft-status-${metaDraftState.kind}`} role={metaDraftState.kind === "completed" ? "status" : "alert"}>{metaDraftState.message}</p>}</article>
        </div>
