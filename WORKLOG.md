...
## 2026-08-26 — 머지 전 생성 진행 반응성 원본 복구

- 목적: 최종 amend 과정에서 제거된 머지 전 진행 화면 반응성 구현을 메인과 발표용에 동일하게 복구한다.
- 변경: reflog 원본 `424b6fa`의 ambient 배경, 일러스트 부유, 확산 링·spark, 실시간 상태·대기 점, 흐르는 진행선, 활성 단계 pulse, 카드 scan과 모바일 구성을 그대로 복구했다. 생성·게시 응답 기반 단계 전환은 유지했다.
