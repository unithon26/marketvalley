...
## 2026-08-26 — Meta PAUSED 초안 운영 생성과 결제 계정 불일치 확인

- 목적: 운영 제품이 회사 Meta 자산에 광고 초안을 자동 생성하는지 검증하고, 사용자가 Instagram 앱에 충전한 잔액으로 마지막 광고 객체까지 만들 수 있는지 확인한다.
- 운영 결과: PNG 5장, 캠페인 `23859320159180798`, 광고 세트 `23859320163730798`, 크리에이티브 `3490743444433659`를 기존 durable operation의 checkpoint를 보존한 채 생성했다. 캠페인과 광고 세트는 `PAUSED`, 광고계정 지출은 `₩0`이며 Ads Manager에서 `Market Valley 시장검증 [68218af0fb]` 캠페인이 꺼짐으로 표시되는 것을 확인했다.
- 실패와 증거: 마지막 광고 객체는 Meta 오류 code `100`, subcode `1359188`, `No Payment Method`로 거절돼 operation을 `RECONCILIATION_REQUIRED`의 `ad` 단계에 남겼다. 자동화 광고계정 `1026341707121609`의 청구 화면은 잔액 `₩0`, 결제수단 없음, 비즈니스 이름·주소 미완료를 표시했다. Instagram 앱의 별도 청구 계정 잔액 `₩4,761`은 Business Settings의 연결 가능 자산이나 자동화 광고계정 Graph 조회에 나타나지 않았다.
- 결정과 검증: 실패 단계만 재개할 수 있도록 성공 checkpoint를 유지하고 자동 재시도·중복 생성·`ACTIVE` 전환은 하지 않았다. 현재 촬영 가능한 사실 범위는 Meta Ads Manager에 나타난 `PAUSED` 캠페인·광고 세트·크리에이티브 자동 생성이며, 실제 게재는 아니다.
- 남은 일: 계정 소유자가 실제 법적 청구 정보와 결제수단을 자동화 광고계정에 등록한 뒤 같은 operation의 `ad` 단계만 재개한다. 최종 광고의 `PAUSED`, 중복 없음과 지출 `₩0`을 확인하기 전에는 완료로 표시하지 않는다.
