...
## 2026-08-26 — 심사용 시스템 아키텍처 단순화

- 목적: 제품을 처음 접하는 심사위원도 아이디어 입력부터 광고 생성, 예약 수집과 사람의 판단까지 한눈에 이해할 수 있는 시스템 아키텍처를 만든다.
- 변경: GitHub CI, Oracle, health, Meta P1과 fallback 등 심사 설명에 불필요한 운영 요소를 제외했다. `서비스 사용자 → 웹앱 → AI 광고 기획 → 하나의 광고 설계서 → 콘텐츠 자동 제작`과 `랜딩 방문자 → 예약 보호 → 로그인·데이터 저장 → 결과 리포트 → 사람의 판단` 두 흐름만 16:9 SVG와 1920×1080 PNG에 남겼다. 기술명은 작은 보조 표기로 제한하고 `docs/architecture.md`의 핵심 설계 의도도 사용자 가치 중심으로 고쳤다.
- 검증: SVG XML 유효성, PNG 1920×1080 크기, 실제 렌더링에서 한글·화살표·저장 경로·판단 순서와 Markdown 참조를 확인했다. 애플리케이션 동작 변경이 없어 코드 테스트는 실행하지 않았다.
- 전달: 제품 구현과 진행 중인 다른 작업을 건드리지 않는 독립 문서 PR로 전달한다. 애플리케이션 배포는 변경하지 않는다.
