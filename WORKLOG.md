...
## 2026-08-26 — 제출용 시스템 아키텍처 도식화

- 목적: 구현·운영·장애 대응 기록에 흩어진 현재 구조를 제출 문서에 바로 삽입할 수 있는 한 장의 시스템 아키텍처로 정리한다.
- 변경: 사용자와 방문자, GitHub CI, Vercel·Oracle 이중 배포, Next.js BFF, `CampaignSpec`, 결정적 렌더러, Anthropic·Supabase·Turnstile과 선택적 Meta `PAUSED` 경로를 16:9 SVG와 1920×1080 PNG로 표현했다. `docs/architecture.md`에 이미지와 핵심 설계 의도 한 줄을 삽입하고 운영 상태 기준일을 2026-08-26으로 갱신했다.
- 검증: SVG XML 유효성, PNG 1920×1080 크기, 실제 렌더링의 한글·연결선·범례 가독성과 Markdown 참조를 확인했다. 애플리케이션 동작 변경이 없어 코드 테스트는 실행하지 않았다.
- 전달: 기존 진행 화면 구현을 건드리지 않고 문서와 이미지 4개 경로만 커밋해 최종 PR #11 브랜치에 push했다. 애플리케이션 배포는 변경하지 않았다.
