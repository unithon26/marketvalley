...
- 남은 일: Cloudflare·Vercel 브라우저 로그인 뒤 Turnstile key와 Vercel GitHub 비공개 저장소 권한을 연결하고 Supabase·Google production redirect를 등록한다. 실제 main SHA를 두 대상에 배포하고 production OAuth·Claude·예약·export·rollback과 재부팅 복구를 검증한다. Oracle 4·24 capacity는 별도로 재시도한다. Meta 자동화는 제외한다.
