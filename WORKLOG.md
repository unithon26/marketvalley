...
- 검증: 기존 구현 기준 lint·typecheck·단위 테스트 31파일 158개, production build와 Chromium E2E 20개가 통과했다. 최신 `main` 인프라를 병합한 뒤 새 Supabase Turnstile 필수 계약을 Meta 테스트 fixture에 반영했고, 최종 lint·typecheck·단위 테스트 39파일 207개, configured server-secret client bundle smoke, production build와 Chromium E2E 21개가 통과했다. token·App Secret 패턴은 저장소에 기록하지 않았다.
- 전달: 구현 커밋 `7932348`을 `codex/meta-p1`에 push하고 PR `#4`를 열었다. 최초 Actions run `32856320241`은 코드 실행 전 `startup_failure`였으며, 원인은 브랜치의 이전 tag 기반 action이 저장소의 SHA pinning 정책에 걸린 것이었다. 최신 `main`의 SHA 고정 workflow와 인프라를 병합했고 새 CI 확인이 남아 있다.
- 남은 일: Business Settings에서 앱 역할을 System User에 연결한 뒤 `ads_management`·`ads_read`만 가진 60일 token을 발급하고, 광고 계정 숫자 ID·KRW·Asia/Seoul과 Page–Instagram 쌍을 다시 확인한다. 이후 명시적 production 변경 승인 아래 migration 적용, Oracle secret 등록, Graph 자산 조회, `PAUSED` 단일 종단·중복 방지·0원 지출을 검증한다. token, App Secret, 결제수단, `ACTIVE`, 실제 광고 객체는 아직 만들지 않았다.
