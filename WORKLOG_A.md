...
- 변경: 리포트 지표 레퍼런스(`proo-landing.vercel.app`)를 검토하는 과정에서, 기존 긍정·중립·부정 익명 신호를 이름+이메일 예약자명단으로 바꾸는 방향이 `docs/decisions/0001-close-the-validation-loop.md`·`docs/validation.md`의 "개인정보 비수집" 원칙 및 `WORKLOG.md`에 기록된 개발자 B의 기존 반려 결정과 충돌함을 발견. 사용자에게 직접 확인 후 의도적 전환으로 확정하고 `docs/decisions/0012-switch-anonymous-signal-to-named-reservation.md`(ADR-0012)를 새로 작성. `docs/spec.md`(P0-4, 지표 정의, Supabase 스키마 스케치)와 `docs/validation.md`(안전성과 진실성)를 ADR-0012 기준으로 갱신. 상세 데이터 계약·화면 변경 범위·작업 순서·A/B 분담을 `docs/superpowers/specs/2026-08-25-reservation-list-migration-design.md`에 기록
- 영향 범위: `docs/decisions/0012-switch-anonymous-signal-to-named-reservation.md`, `docs/spec.md`, `docs/validation.md`, `docs/superpowers/specs/2026-08-25-reservation-list-migration-design.md`, `WORKLOG_A.md`, `TROUBLESHOOTING_A.md`. 제품 코드(`app/`, `components/`, `lib/`)는 이번 세션에서 변경하지 않음
...
- 검증: 문서 전용 변경이라 `pnpm check`는 실행하지 않음. `docs/superpowers/specs/2026-08-24-figma-alignment-and-dependency-split-design.md`를 대조해, 디자이너 원본 Figma 리포트 화면도 원래 노출수·CTR·예약률·이메일 리스트 구성이었고 어제 감사에서 개인정보 비수집 원칙 때문에 의도적으로 이탈했던 것임을 확인 — 이번 전환이 그 Figma 원안으로 되돌아가는 결정이라는 근거를 ADR-0012와 마이그레이션 설계 문서에 남김
